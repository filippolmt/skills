...
skills installable via git. Two routes to the same skills: Claude installs them
as plugins, **referenced** upstream one by one; and a generated tree of **vendored
copies** at `skills/` serves all three harnesses at once, which is also what makes
this repo itself one installable plugin (see
...
.claude-plugin/plugin.json         # this repo is ALSO one plugin, whose skills are skills/
skills/                            # generated: vendored copies. The one tree, read by all three
  tdd/
...
## The tree: one copy, three harnesses

`skills/` holds a vendored copy of each catalog skill at its pinned `sha`,
generated by `node scripts/gen-skills-tree.js`. That position is not arbitrary —
it is the only one all three agents can reach:

| | how it finds `skills/` | to install |
|---|---|---|
| Claude | a plugin's mandatory `<plugin-root>/skills/`, and this repo is a plugin | `/plugin install skills-tree` |
...
**One route or the other, on Claude.** `skills-tree` ships the same skills the
catalog offers individually. Installing both gives Claude two copies of each — pick
the tree, or pick the entries.

**Everywhere at once, no install** — one symlink covers pi and Codex, and `git pull`
is the update:
...
- `skills-tree` — Every vendored skill in one plugin — the same tree at `skills/` that pi and Codex read directly. Install this OR the individual entries, not both.
