...
skills installable via git. Claude installs them as plugins, **referenced**
upstream one by one; pi and Codex read a generated tree of **vendored copies** at
`skills/`, because neither can address a subdirectory of somebody else's
repository (see [ADR-0010](docs/adr/0010-vendor-a-shared-skills-tree-on-main.md)).
...
.agents/skills -> ../skills        # symlink: the standard path pi and Codex scan by themselves
...
`skills/` holds a vendored copy of each catalog skill at its pinned `sha`,
generated by `node scripts/gen-skills-tree.js`. It exists because neither pi nor
Codex can install a subdirectory of somebody else's repository — the shape every
`git-subdir` entry has. Claude does not use it: it installs from the catalog,
referenced upstream, and that route stays the better one for Claude — per-skill,
and with nothing redistributed.

| | how it finds `skills/` | to install |
|---|---|---|
| pi | its package **convention directory**, `<root>/skills` | `pi install git:github.com/filippolmt/skills` |
| pi and Codex, inside a checkout | the `.agents/skills` symlink — the Agent Skills standard's shared path | **nothing** |

**Everywhere at once, no install** — one symlink covers both agents, and `git pull`
is the update:
